...
from qibolab._core.instruments.emulator.engine.dynamiqs import _spline_function

PLATFORMS_DIR = Path(__file__).parent / "platforms"
PLATFORM_NAMES = sorted(p.name for p in PLATFORMS_DIR.iterdir() if p.is_dir())


def test_dynamiqs_engine_is_publicly_exported():
    assert PublicDynamiqsEngine is DynamiqsEngine
