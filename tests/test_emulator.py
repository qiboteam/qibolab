...
@pytest.mark.parametrize(
    "acquisition", [AcquisitionType.DISCRIMINATION, AcquisitionType.INTEGRATION, AcquisitionType.RAW]
)
...
def test_emulator_execute_pulse_sequence_fast_reset(name):
    ...
    sequence = PulseSequence()
    sequence.add(platform.create_qubit_readout_pulse(0, 0))
    options = ExecutionParameters(nshots=None, fast_reset=True) #fast_reset does nothing in emulator
    result = platform.execute_pulse_sequence(sequence, options)
...
def test_emulator_single_sweep(name, fast_reset, parameter, average, acquisition, nshots):
    runcard_folder = f'{emulator_test.__path__[0]}/{name}'
    platform = create_oneQ_emulator(runcard_folder)
    sequence = PulseSequence()
    ...
    if parameter in platform.instruments['pulse_simulator'].available_sweep_parameters:
        ...
        if average:
            results_shape = (
                results[pulse.qubit].statistical_frequency.shape
            )
        else:
            results_shape = (
                results[pulse.qubit].samples.shape
            )
...
@pytest.mark.parametrize("nshots", [10, 20])
def test_emulator_double_sweep(name, parameter1, parameter2, average, acquisition, nshots):
    runcard_folder = f'{emulator_test.__path__[0]}/{name}'
    platform = create_oneQ_emulator(runcard_folder)
    sequence = PulseSequence()
    ...
    if parameter1 in platform.instruments['pulse_simulator'].available_sweep_parameters and parameter2 in platform.instruments['pulse_simulator'].available_sweep_parameters:
...
def test_emulator_single_sweep_multiplex(name, parameter, average, acquisition, nshots):
    ...
    if parameter in platform.instruments['pulse_simulator'].available_sweep_parameters:
        ...
        for ro_pulse in ro_pulses.values():
            ...
            if average:
                results_shape = (
                    results[ro_pulse.qubit].statistical_frequency.shape
                )
            else:
                results_shape = (
                    results[ro_pulse.qubit].samples.shape
                )
            assert results_shape == (SWEPT_POINTS,) if average else (nshots, SWEPT_POINTS)
