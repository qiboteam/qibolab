...
def create():
    ...
    channels |= (f"flux-c{c}" for c in couplers)
    ...
    for c, coupler in couplers.items():
        ##coupler.drive = channels[f"drive-{c}"]
        coupler.flux = channels[f"flux-c{c}"]

        ##channels[f"drive-{c}"].coupler = coupler
        channels[f"flux-c{c}"].coupler = coupler
        coupler.sweetspot = 0  # not used
