...
def test_register_init_shifted_by_updparam_after_wait_register():
    # Simulate a program where a register is initialized, then used in Wait(Register), then UpdParam follows
    regnum = 7
    block = [
        Move(source=100, destination=Register(number=regnum)),
