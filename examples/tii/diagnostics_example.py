...
for qubit in platform.qubits:
    ...
    if save_settings:
        ...
        ds.save_config_parameter(
            "resonator_spectroscopy_avg_min_ro_voltage",
    ...
    if save_settings:
        ...
        RX_pulse_sequence = platform.settings["native_gates"]["single_qubit"][qubit][
            "RX"
        ]["pulse_sequence"]
    ...
    (
        ...
        t1,
    ) = ds.run_rabi_pulse_length(qubit)
    if save_settings:
        ...
        ds.save_config_parameter(
            "rabi_oscillations_pi_pulse_min_voltage",
            float(rabi_oscillations_pi_pulse_min_voltage),
            "characterization",
            "single_qubit",
            qubit,
        )
    ...
    if save_settings:
        ds.save_config_parameter(
            "T1", float(t2), "characterization", "single_qubit", qubit
        )
    ...
    if save_settings:
        ...
        ds.save_config_parameter(
            "qubit_freq", adjusted_qubit_freq, "characterization", "single_qubit", qubit
        )
        ds.save_config_parameter(
    ...
    (
        ...
        all_exc_states,
        mean_exc_states,
    ) = ds.calibrate_qubit_states(qubit)
    if save_settings:
