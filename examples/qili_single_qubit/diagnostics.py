...
def get_config_parameter(dictID, dictID1, key):
    ...
    calibration_path = os.path.realpath(
        os.path.join(
            os.path.dirname(__file__),
            "..",
            "..",
...
def plot(smooth_dataset, dataset, label, type):
    if type == 0:  # cavity plots
        ...
        ax.plot(
            dataset["x0"].values[smooth_dataset.argmax()],
            smooth_dataset[smooth_dataset.argmax()],
            "o",
            color="C2",
        )
    ...
    if type == 1:  # qubit spec, rabi, ramsey, t1 plots
        ...
        ax.title.set_text(label)
        ax.plot(
            dataset["x0"].values[smooth_dataset.argmin()],
            smooth_dataset[smooth_dataset.argmin()],
            "o",
            color="C2",
        )
        plt.savefig(pathlib.Path("data") / f"{label}.pdf")
...
def run_punchout(platform, mc, sequence, ro_pulse, precision_width, precision_step):
    ...
    mc.settables(
        [
            Settable(platform.LO_qrm.device.frequency),
...
def run_rabi_pulse_length_and_gain(
    ...
    mc.settables(
        [
            Settable(QCPulseLengthParameter(ro_pulse, qc_pulse)),
