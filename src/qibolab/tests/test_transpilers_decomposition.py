...
def assert_single_qubits(psi, ua, ub):
    ...
    for i, j in zip(range(4), [0, 1, 3, 2]):
        ...
        target_state = native.magic_basis[j]
...
def test_eigenbasis_entanglement(run_number):
    ...
    states, eigvals = native.calculate_psi(unitary)
...
def test_v_decomposition(run_number):
    ...
    unitary = random_unitary(2)
    psi, eigvals = native.calculate_psi(unitary)
    va, vb = native.calculate_single_qubit_unitaries(psi)
...
def test_u_decomposition(run_number):
    ...
    ua_dagger, ub_dagger = native.calculate_single_qubit_unitaries(psi_tilde)
...
def test_calculate_h_vector(run_number):
    ...
    unitary = random_unitary(2)
    ua, ub, ud, va, vb = native.magic_decomposition(unitary)
    hx, hy, hz = native.calculate_h_vector(ud)
    ...
    target_matrix = expm(-1j * ham)
    np.testing.assert_allclose(ud, target_matrix, atol=1e-12)
