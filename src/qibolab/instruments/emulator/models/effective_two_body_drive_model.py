...
# model template for 0-1 system with qubit 2 as traced out coupler but provides drive channel for 2q gates
...
def get_model_params(
    ...
    """
    model_params_dict = {"model_name": "effective_two_body_drive_model"}
...
def generate_model_config(
    ...
    actual_device_qubits_list = np.unique(np.array(topology).flatten())
    actual_device_qubits_list = [str(q) for q in actual_device_qubits_list]
    ...
    two_body_drive_hamiltonian_dict = {}
    ...
    for q in actual_device_qubits_list:
    ...
    ):  # consistent with actual_device_qubits_list
        ind2, ind1 = key.split(
            "_"
    ...
    for key in exception_qubits:
        ...
        targets = two_body_drive_dict[key]
        ind2, ind1 = targets.split(
            "_"
        )  # ind2 > ind1 with ind_qubit > ind_coupler as per Hilbert space ordering
        ind1 = platform_to_simulator_qubits[ind1]
        ind2 = platform_to_simulator_qubits[ind2]

        two_body_drive_hamiltonian_dict.update({f"D-{ind_key}": []})
        two_body_drive_hamiltonian_dict[f"D-{ind_key}"].append(
            (
                2 * np.pi * model_params["rabi_freq"][key], 
                (
                    f"sig01_{ind2} ^ sig10_{ind1} + sig10_{ind2} ^ sig01_{ind1}"
                    f" + sig10_{ind2} ^ sig12_{ind1} + sig12_{ind2} ^ sig10_{ind1}"
                    f" + sig01_{ind2} ^ sig21_{ind1} + sig21_{ind2} ^ sig01_{ind1}"
                ),
                [ind2, ind1]
            )
        )
    ...
    model_config = {
        ...
        "two_body_drive": two_body_drive_hamiltonian_dict,
    }
