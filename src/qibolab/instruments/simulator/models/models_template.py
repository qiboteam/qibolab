...
def generate_model_config(
    model_params: Union[dict, str] = None, nlevels_q: list = None, nlevels_c: list = None, topology:list=None
) -> dict:
    ...
    for i, q in enumerate(qubits_list):
        ...
        g1 = 0 if t1==0 else 1./t1
        t2 = model_params['T2'][q]
        g2 = 0 if t1==0 else 1./t2
    ...
    for i,c in enumerate(couplers_list):
        # drift Hamiltonian terms (constant in time)
        drift_hamiltonian_dict['one_body'].append((2 * np.pi * model_params['lo_freq'][c]/GHz, f'O_{c}', [c]))
        drift_hamiltonian_dict['one_body'].append((np.pi * model_params['anharmonicity'][c]/GHz, f'O_{c} * O_{c} - O_{c}', [c]))
