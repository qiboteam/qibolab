...
from qibolab.instruments.simulator.models.methods import load_model_params, MHz, GHz, default_noflux_platform2simulator_channels 


def generate_model_config(model_params:Union[dict, str], nlevels_q:list=None, topology:list=None)->dict:    
    ...
    for i, q in enumerate(qubits_list):
        ...
        drift_hamiltonian_dict['one_body'].append((2 * np.pi * model_params['lo_freq'][q]/GHz, f'O_{q}', [q]))
        drift_hamiltonian_dict['one_body'].append((np.pi * model_params['anharmonicity'][q]/GHz, f'O_{q} * O_{q} - O_{q}', [q]))
        
        # drive Hamiltonian terms (amplitude determined by pulse sequence)
        drive_hamiltonian_dict.update({f'D-{qubits_list[i]}':[]})
        drive_hamiltonian_dict[f'D-{qubits_list[i]}'].append((2 * np.pi * model_params['rabi_freq'][q]/GHz, f'X_{q}', [q]))

        # dissipation terms (one qubit, constant in time)
        t1 = model_params['T1'][q]
        g1 = 0 if t1==0 else 1./t1 * 2*np.pi/GHz
        t2 = model_params['T2'][q]
        g2 = 0 if t1==0 else 1./t2 * 2*np.pi/GHz
    ...
    for key in list(model_params["coupling_strength"].keys()):
        ind2, ind1 = key.split('_') # ind2 > ind1 with ind_qubit > ind_coupler as per Hilbert space ordering
        coupling = model_params["coupling_strength"][key]
        drift_hamiltonian_dict['two_body'].append((2 * np.pi * coupling/GHz, f'bdag_{ind2} ^ b_{ind1} + b_{ind2} ^ bdag_{ind1}',[ind2, ind1]))        
    ...
    model_config = {
        ...
        'platform2simulator_channels': default_noflux_platform2simulator_channels(qubits_list, couplers_list=[]),
    }
...
def generate_model_config_oneQ(model_params:dict, nlevels_q:int=None)->dict:
    """Calls `generate_model_config` to construct a model configuration dictionary for generic 1Q emulators.
    """
    model_config = generate_model_config(model_params, nlevels_q=[nlevels_q], topology=[])
