...
def dec_to_basis_string(x:int, nlevels:list=[2], to_string:bool=True) -> Union[str, list]:
    """Converts an integer to a generalized bitstring in the computation basis of the full Hilbert space.
...
def make_comp_basis(qubit_list:List[Union[int,str]], qid_nlevels_map:dict[Union[int,str],int]) -> np.ndarray:
...
def op_from_instruction(
    inst: tuple[float,str,list], op_dict: Optional[dict] = None, op_connectors_dict: Optional[dict] = None, multiply_coeff: bool = True,
):
    ...
    def process_op(
        op_list:list, connector_list:List[str], connector:str
    )->list:
...
def print_Hamiltonian(model_config, op_qid_list:list=None):
    ...
    def format_instruction(op_instruction, is_dissipation=False):
        ...
        if is_dissipation:
            ...
            return [f'${op_instruction[1]}$', f'${op_instruction[0]/np.sqrt(2*np.pi)}~{ghz_units}$']            
        else:
            ghz_units = '\\text{GHz}'
            return [f'${op_instruction[1]}$', f'${op_instruction[0]/2/np.pi}~{ghz_units}$']            
    ...
    for k in op_qid_list:
        ...
        latex_op_dict["bdag"].update({k: f"b^{{\dagger}}_{k}"})
        latex_op_dict["O"].update({k: f"O_{k}"})
        latex_op_dict["X"].update({k: f"X_{k}"})
        latex_op_dict["Z01"].update({k: f"\sigma^Z_{k}"})
        latex_op_dict["sp01"].update({k: f"\sigma^+_{k}"})

    latex_op_connectors_dict = OrderedDict(
        [
            ("^", lambda a, b: f"{a}{{\otimes}}{b}"),
            ("*", lambda a, b: f"{a}{b}"),
            ("+", lambda a, b: f"{a}+{b}"),
            ("-", lambda a, b: f"{a}-{b}"),
        ]
    ...
    basic_dict = [
        f'$O_i = b^{{\dagger}}_i b_i$',
        f'$X_i = b^{{\dagger}}_i + b_i$',
    ]
    print('Dictionary')
    for i in basic_dict:
        display(Latex(i))
    ...
    for op_instruction in model_config["drift"]["one_body"]:
        op_instruction = op_from_instruction(op_instruction, latex_op_dict, latex_op_connectors_dict, multiply_coeff=False)
