...
class Cluster(AbstractInstrument):
    ...
    Attributes:
        is_connected: bool = Indicates the connection status.
    ...
    def __init__(self, name, address):
        ...
        self.reference_clock_source: str = 'internal'
    ...
    def setup(self, **kwargs):
        ...
        if self.is_connected:
            ...
            self.device.set('reference_source', self.reference_clock_source)
        else:
    ...
    def connect(self):
        ...
        if not self.is_connected:
            if cluster:
                ...
                self._set_device_parameter(self.device, 'out0_offset_path0', 'out0_offset_path1', value = 0) # Default after reboot = 7.625
                self._set_device_parameter(self.device, 'scope_acq_avg_mode_en_path0', 'scope_acq_avg_mode_en_path1', value = True)
                self._set_device_parameter(self.device, 'scope_acq_sequencer_select', value = self.DEFAULT_SEQUENCERS['o1']) 
                self._set_device_parameter(self.device, 'scope_acq_trigger_level_path0', 'scope_acq_trigger_level_path1', value = 0) 
                self._set_device_parameter(self.device, 'scope_acq_trigger_mode_path0', 'scope_acq_trigger_mode_path1', value = 'sequencer') 
                ...
                self._set_device_parameter(target, 'cont_mode_waveform_idx_awg_path0', 'cont_mode_waveform_idx_awg_path1', value = 0)
                ...
                for sequencer in range(1,  self._device_num_sequencers):
                    self._set_device_parameter(self.device.sequencers[sequencer], 'channel_map_path0_out0_en', 'channel_map_path1_out1_en', value = False) # Default after reboot = True
            ...
            if not port_pulses.is_empty:
                ...
                # TODO: for non_overlapping_same_frequency_pulses in non_overlapping_pulses.separate_different_frequency_pulses():
                    ...
                    if len(self._free_sequencers_numbers) == 0:
                        raise Exception(f"The number of sequencers requried to play the sequence exceeds the number available {self._device_num_sequencers}.")
                        ...
                        # if there is not enough memory in the current sequencer, use another one
                            ...
                            for parameter in self.device.sequencers[self.DEFAULT_SEQUENCERS[port]].parameters:
                                ...
                                if not value is None:
                                    target  = self.device.sequencers[next_sequencer_number]
                                    self._set_device_parameter(target, parameter, value = value)
                            self._set_device_parameter(self.device.sequencers[next_sequencer_number], 'nco_freq', value = pulse.frequency)  
                            sequencer = Sequencer(next_sequencer_number)
        ...
        for port in self._output_ports_keys:                
            for sequencer in self._sequencers[port]:
                ...
                for pulse in sequencer.pulses.ro_pulses:
                    ...
                    i, q = self._process_acquisition_results(raw_results[acquisition_name], pulse, demodulate = (not self.ports['i1'].hardware_demod_en))
    ...
    def connect(self):
        ...
        if not self.is_connected:
            if cluster:
                ...
                self.is_connected = True
                self._set_device_parameter(self.device, 'out0_offset_path0', 'out0_offset_path1', value = 0) # Default after reboot = 7.625
                self._set_device_parameter(self.device, 'out1_offset_path0', 'out1_offset_path1', value = 0) # Default after reboot = 7.625
                ...
                self._set_device_parameter(self.device.sequencers[self.DEFAULT_SEQUENCERS['o1']], 'channel_map_path0_out0_en', 'channel_map_path1_out1_en', value = True)
                self._set_device_parameter(self.device.sequencers[self.DEFAULT_SEQUENCERS['o1']], 'channel_map_path0_out2_en', 'channel_map_path1_out3_en', value = False)
                self._set_device_parameter(self.device.sequencers[self.DEFAULT_SEQUENCERS['o2']], 'channel_map_path0_out0_en', 'channel_map_path1_out1_en', value = False)
                self._set_device_parameter(self.device.sequencers[self.DEFAULT_SEQUENCERS['o2']], 'channel_map_path0_out2_en', 'channel_map_path1_out3_en', value = True)
                self.device_num_sequencers = len(self.device.sequencers)
                for sequencer in range(2,  self.device_num_sequencers):
                    self._set_device_parameter(self.device.sequencers[sequencer], 'channel_map_path0_out0_en', 'channel_map_path1_out1_en', value = False) # Default after reboot = True
                    self._set_device_parameter(self.device.sequencers[sequencer], 'channel_map_path0_out2_en', 'channel_map_path1_out3_en', value = False) # Default after reboot = True
