...
class Zurich(AbstractInstrument):
    ...
    def setup(self, qubits, couplers, relaxation_time=0, time_of_flight=0, smearing=0, **_kwargs):
        ...
        for coupler in couplers.values():
            if coupler.flux:
                self.register_flux_coupler_line(coupler)
    ...
    def register_coupler_flux_line(self, coupler):
        """Registers qubit flux coupler line to calibration and signal map."""
        c = coupler.name
        self.signal_map[f"flux_coupler{c}"] = self.device_setup.logical_signal_groups[f"c{c}"].logical_signals[
            "flux_coupler_line"
        ]
        self.calibration[f"/logical_signal_groups/c{c}/flux_coupler_line"] = lo.SignalCalibration(
            range=coupler.flux.power_range, port_delay=0, delay_signal=0, voltage_offset=coupler.flux.offset
        )
    ...
    def play(self, qubits, couplers, sequence, nshots, relaxation_time):
        if relaxation_time is None:
            self.relaxation_time = 10.0e-6
        else:
            self.relaxation_time = relaxation_time

        self.sequence_zh(sequence)
        self.create_exp(qubits, couplers, nshots)
        self.run()
    ...
    def create_exp(self, qubits, couplers, nshots, **Params):
        # def create_exp(self, qubits, couplers, **Params):
        signals = []
        ...
        for coupler in couplers.values():
            signals.append(lo.ExperimentSignal(f"flux_coupler{coupler.name}"))
        ...
        with exp.section(uid=f"sequence_flux_bias", alignment=lo.SectionAlignment.RIGHT):
            for qubit in qubits.values():
                ...
                for pulse in self.sequence[f"flux{qubit.name}"]:
                    exp.delay(signal=f"flux{qubit.name}", time=pulse.pulse.start - time)
                    time += pulse.pulse.duration
        ...
        with exp.section(uid=f"sequence_drive", alignment=lo.SectionAlignment.RIGHT):
            for qubit in qubits.values():
                ...
                for pulse in self.sequence[f"drive{qubit.name}"]:
                    exp.delay(signal=f"drive{qubit.name}", time=pulse.pulse.start - time)
                    time += pulse.pulse.duration
                    pulse.zhpulse.uid = pulse.zhpulse.uid + "i"
        ...
        with exp.section(uid=f"sequence_measure"):
            for qubit in qubits.values():
                ...
                for pulse in self.sequence[f"measure{qubit.name}"]:
                    exp.delay(signal=f"measure{qubit.name}", time=pulse.pulse.start - time)
                    time += pulse.pulse.duration
                    pulse.zhpulse.uid = pulse.zhpulse.uid + "i"
