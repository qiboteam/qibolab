...
class Calibration():
    ...
    def run_rabi_pulse_length(self, qubit=0):
        ...
        if self.resonator_type == '3D':
            pi_pulse_duration, rabi_oscillations_pi_pulse_peak_voltage = fitting.rabi_fit_3D(dataset)
        elif self.resonator_type == '2D':
            pi_pulse_duration, rabi_oscillations_pi_pulse_peak_voltage = fitting.rabi_fit_2D(dataset)
    ...
    def run_rabi_pulse_gain(self, qubit=0):
        ...
        if self.resonator_type == '3D':
            pi_pulse_gain, rabi_oscillations_pi_pulse_peak_voltage = fitting.rabi_fit_3D(dataset)
        elif self.resonator_type == '2D':
            pi_pulse_gain, rabi_oscillations_pi_pulse_peak_voltage = fitting.rabi_fit_2D(dataset)
    ...
    def run_rabi_pulse_amplitude(self, qubit=0):
        ...
        self.reload_settings()
        pulse_amplitude_start = self.settings['rabi_pulse_amplitude']['pulse_amplitude_start']
        pulse_amplitude_end = self.settings['rabi_pulse_amplitude']['pulse_amplitude_end']
        ...
        if self.resonator_type == '3D':
            ...
            pi_pulse_amplitude, rabi_oscillations_pi_pulse_peak_voltage = fitting.rabi_fit_2D(dataset)
    ...
    def run_ramsey(self, qubit=0):
        ...
        RX90_pulse2 = platform.RX90_pulse(qubit, start = RX90_pulse1.duration, phase = RX90_pulse1.duration / sampling_rate * 2 * np.pi * RX90_pulse1.frequency)
        ...
        mc.settables(Settable(RamseyWaitParameter(ro_pulse, RX90_pulse2, sampling_rate)))
        ...
        return delta_frequency, corrected_qubit_frequency, int(t2), smooth_dataset, dataset
    ...
    def run_ramsey_frequency_detuned(self, qubit):
        ...
        runcard_qubit_freq = platform.settings['characterization']['single_qubit'][qubit]['qubit_freq']
        ...
        for t_max in t_end:
            ...
            mc.settables(RamseyFreqWaitParameter(ro_pulse, RX90_pulse2, offset_freq, sampling_rate))
        ...
        return new_t2, (corrected_qubit_freq - runcard_qubit_freq), corrected_qubit_freq, dataset
    ...
    def run_rabi_pulse_length_and_amplitude(self, qubit=0):
        ...
        mc.settables([QCPulseLengthParameter(ro_pulse, qd_pulse), QCPulseAmplitudeParameter(qd_pulse)])
        setpoints_length = np.arange(pulse_duration_start, pulse_duration_end, pulse_duration_step)
        setpoints_amplitude = np.arange(pulse_amplitude_start, pulse_amplitude_end, pulse_amplitude_step)
        mc.setpoints_grid([setpoints_length, setpoints_amplitude])
        mc.gettables(ROController(platform, sequence, qubit))

        platform.start()
        dataset = mc.run('Rabi Pulse Length and Amplitude', soft_avg = self.software_averages)
        platform.stop()
    ...
    def run_spin_echo(self, qubit=0):
        ...
        RX_pulse = platform.RX_pulse(qubit, start = RX90_pulse.duration, phase = RX90_pulse.duration / sampling_rate * 2 * np.pi * RX90_pulse.frequency)
    ...
    def run_spin_echo_3pulses(self, qubit=0):
        ...
        mc.settables(SpinEcho3PWaitParameter(ro_pulse, RX_pulse, RX90_pulse2, sampling_rate))
        mc.setpoints(np.arange(self.delay_between_pulses_start, self.delay_between_pulses_end, self.delay_between_pulses_step))
        mc.gettables(Gettable(ROController(platform, sequence, qubit)))
        platform.start()
    ...
    def run_drag_pulse_tunning(self, qubit):
        ...
        for beta_param in np.arange(self.beta_start, self.beta_end, self.beta_step).round(4): 
            ...
            #drag pulse RY(pi)
            RY_drag_pulse = platform.RX_drag_pulse(qubit, start = RX90_drag_pulse.duration, phase = RX90_drag_pulse.duration / sampling_rate * (2 * np.pi) * RX90_drag_pulse.frequency + np.pi/2, beta=beta_param)            
            #RO pulse
            ...
            #drag pulse RY(pi)
            RY_drag_pulse = platform.RX_drag_pulse(qubit, start = 0, phase = np.pi/2, beta=beta_param)
            #drag pulse RX(pi/2)
            RX90_drag_pulse = platform.RX90_drag_pulse(qubit, start = RY_drag_pulse.duration, phase = RY_drag_pulse.duration / sampling_rate * (2 * np.pi) * RX90_drag_pulse.frequency, beta = beta_param)
...
class RamseyWaitParameter():
    ...
    def set(self, value):
        ...
        self.qc2_pulse.phase = (self.qc2_pulse.start / self.sampling_rate) * (2 * np.pi) * self.qc2_pulse.frequency
...
class SpinEcho3PWaitParameter():
    ...
    def set(self,value):
        ...
        self.qc3_pulse.phase = (self.qc3_pulse.start / self.sampling_rate) * (2 * np.pi) * self.qc3_pulse.frequency
...
class RamseyFreqWaitParameter():
    ...
    def set(self, value):
        self.qc2_pulse.start = self.pulse_length + value
        self.qc2_pulse.phase = (self.qc2_pulse.start / self.sampling_rate) * (2 * np.pi) * (self.qc2_pulse.frequency - self.offset_freq)
        self.ro_pulse.start = self.pulse_length * 2 + value
