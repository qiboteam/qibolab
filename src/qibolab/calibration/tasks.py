...
from qibolab.pulses import Pulse, IQReadoutPulse
...
def PulseSpectroscopy(scan_start: float, scan_stop: float, scan_step: float, qubit_channel: int, readout_channels: Tuple[int], readout_if: float,
    ...
    for freq in sweep:
        drive = Pulse(qubit_channel, readout_start - pulse_time, pulse_time, 0.75 / 2, freq, 0, Rectangular())
...
def RabiSpectroscopy(scan_start: float, scan_stop: float, scan_step: float, qubit_channel: int, readout_channels: Tuple[int], readout_if: float,
    ...
    for pulse_time in sweep:
        drive = Pulse(qubit_channel, readout_start - pulse_time, pulse_time, 0.75 / 2, qubit_frequency, 0, Rectangular())
...
def RamseyInferometry(scan_start: float, scan_stop: float, scan_step: float, qubit_channel: int, readout_channels: Tuple[int], readout_if: float,
    ...
    sequence = []
    sweep = np.arange(scan_start, scan_stop, scan_step)
    second_pulse = Pulse(qubit_channel, readout_start - pi_half, pi_half, 0.75 / 2, qubit_frequency, 0, Rectangular())
    for tau in sweep:
        first_pulse = Pulse(qubit_channel, readout_start - pi_half - tau - pi_half, pi_half, 0.75 / 2, qubit_frequency, 0, Rectangular())
...
def T1(scan_start: float, scan_stop: float, scan_step: float, qubit_channel: int, readout_channels: Tuple[int], readout_if: float,
    ...
    for tau in sweep:
        if tau >= 0:
            drive = Pulse(qubit_channel, readout_start - tau - pi_pulse, pi_pulse, 0.75 / 2, qubit_frequency, 0, Rectangular())
            sequence.append(PulseSequence([drive, readout]))
        else:
...
def Spinecho(scan_start: float, scan_stop: float, scan_step: float, qubit_channel: int, readout_channels: Tuple[int], readout_if: float,
    ...
    sequence = []
    sweep = np.arange(scan_start, scan_stop, scan_step)
    second_half_pulse = Pulse(qubit_channel, readout_start - pi_half, pi_half, 0.75 / 2, qubit_frequency, 0, Rectangular())
    for tau in sweep:
        tau_half = tau / 2
        start =  readout_start - pi_half - tau_half - pi_pulse
        middle_pulse = Pulse(qubit_channel, start, pi_pulse, 0.75 / 2, qubit_frequency, 0, Rectangular())
        start = start - tau_half - pi_half
        first_half_pulse = Pulse(qubit_channel, start, pi_half, 0.75 / 2, qubit_frequency, 0, Rectangular())
