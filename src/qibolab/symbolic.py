...
class SymbolicExpression:
    ...
    def __init__(self, type: type, expression = 0, symbol: str = ''): # (self, expression: str|{self.type}|SymbolicExpression = 0, symbol: str = ''):
    ...
    def expression(self, expression): # (self, expression: str|{self.type}|SymbolicExpression):
        if isinstance(expression, str):
            ...
            raise TypeError(f"expression argument type should be {self.type.__name__} or SymbolicExpression, got {type(expression).__name__}")
        ...
        for symbol in SymbolicExpression.instances.keys():
            if symbol in expression:
                if symbol in previous_evaluations:
                    raise SymbolicExpression.CircularReferenceError(f"Circular Reference evaluating {expression}, variable {symbol} found in {previous_evaluations}")
                match_string = rf'\b{re.escape(symbol)}\b'
                replacement = str(self.evaluate(SymbolicExpression.instances[symbol]._expression, * previous_evaluations, SymbolicExpression.instances[symbol]._symbol))
                expression = re.sub(match_string, replacement, expression)
        try:
            result = eval(self._sanitise(expression))
        except: 
            raise ValueError(f"The evaluation of the expression: {expression} returned an error")

        if not isinstance(result, self.type):
            raise TypeError(f"The evaluation of the expression: {expression} does not return {self.type.__name__}")
        return result
        ...
        for symbol in SymbolicExpression.instances.keys():
            if symbol.startswith('_sym_') and symbol in expression:
                if symbol in previous_evaluations:
                    raise SymbolicExpression.CircularReferenceError(f"Circular Reference evaluating {expression}, variable {symbol} found in {previous_evaluations}")
                match_string = rf'\b{re.escape(symbol)}\b'
                replacement = str(self._replace_internal_symbols(SymbolicExpression.instances[symbol]._expression, * previous_evaluations, SymbolicExpression.instances[symbol]._symbol))
                expression = re.sub(match_string, replacement, expression)

        return expression

    def _sanitise(self, expression: str) -> str:
    ...
    def __lt__(self, other):
        if isinstance(other, SymbolicExpression):
            ...
            raise TypeError(f"Comparison operators expect SymbolicExpression or {self.type.__name__} arguments, got {type(other).__name__}")
    ...
    def __add__(self, other): # -> SymbolicExpression:
        ...
        elif self.type == float and isinstance(other, self.supported_types) or self.type == int and isinstance(other, int):
