...
def process_acquisition_channel_pulse(
    ...
    if pulse.kind == "delay":
        delay = qcs.Delay(
            duration=sweep_param_map.get("duration", pulse.duration * nano)
        )
        ...
        qcs_pulse = qcs.RFWaveform(
            duration=sweep_param_map.get("duration", pulse.probe.duration * nano),
...
def process_iq_channel_pulse(
    ...
    if pulse.kind == "delay":
        qcs_pulse = qcs.Delay(sweep_param_map.get("duration", pulse.duration * nano))
...
def process_dc_channel_pulse(
    ...
    if pulse.kind == "delay":
        ...
        if pulse.envelope.kind == "rectangular" and pulse.duration * nano > 1e-6:
            qcs_pulse = qcs.DCWaveform.create_dc_flattop(
                rise_duration=20e-9,
                hold_duration=sweep_param_map.get(
                    "duration", pulse.duration * nano - 40e-9
                ),
                fall_duration=20e-9,
            ...
            qcs_pulse = qcs.DCWaveform.create_dc_flattop(
                ...
                hold_duration=sweep_param_map.get(
                    "duration", pulse.duration * nano - risefall_duration * 2
                ),
