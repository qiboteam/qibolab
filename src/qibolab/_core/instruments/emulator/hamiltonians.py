...
>>>>>>> 307f0cea ([pre-commit.ci] auto fixes from pre-commit.com hooks)
        ...
        for i, qubit in self.qubits.items():
            if len(qubit.t1) > 0:
                collapse_operators.append(
                    engine.expand(
                        op=qubit.relaxation(qubit.transmon_levels, engine),
                        dims=self.dims,
                        targets=self.hilbert_space_index(i),
                    )
                )
            if len(qubit.t2) > 0:
                collapse_operators.append(
                    engine.expand(
                        op=qubit.dephasing(qubit.transmon_levels, engine),
                        dims=self.dims,
                        targets=self.hilbert_space_index(i),
                    )
                )
