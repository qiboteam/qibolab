...
def _second_pass(
    ...
    """
    # Wait can either be in a block generated by _decompose_wait or in a block on its
    # own. In the former case the block consists of a remainder (index 0) and a loop
    # (indices 1-3). In that case we want to subtract the duration from the wait at
    ...
    if isinstance(instr, Wait):
        ...
        if new_duration < 0:
            raise ValueError(
                f"Wait duration underflow: attempted to subtract "
                f"{state.nwait_to_subtract[state.nwait]} from original duration "
                f"{instr.duration} (wait index {state.nwait}). This likely means there "
                "are too many consecutive UpdParam (relative phase) instructions, "
                "causing a cumulative time delay that cannot be compensated. "
                "Consider inserting additional Wait/Delay instructions or reducing the "
                "number  of consecutive UpdParam instructions to ensure all wait "
                "durations remain non-negative."
            )
        new_line = Line(
            label=block[0].label,
        ...
        return result, state.model_copy(update={"nwait": state.nwait + 1})
    return block, state
