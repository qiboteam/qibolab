...
def create_tii_qw25q_A(runcard, simulation_duration=None, address=None, cloud=False):
    """Create platform using Quantum Machines (QM) OPXs and Rohde Schwarz/ERAsynth local oscillators.
    ...
    for q in ["A1", "A2", "A4"]:  # Qubits with LO around 7e9
        qubits[q].readout = channels[wiring["readout"][q[0]][0]]
        qubits[q].feedback = channels[wiring["feedback"][q[0]][0]]
    for q in ["A3", "A5", "A6"]:  # Qubits with LO around 7.5e9
        qubits[q].readout = channels[wiring["readout"][q[0]][1]]
        qubits[q].feedback = channels[wiring["feedback"][q[0]][1]]
...
def create_tii_qw25q_B(runcard, simulation_duration=None, address=None, cloud=False):
    ...
    for q in ["B1", "B2", "B3"]:  # Qubits with LO around 7e9
        qubits[q].readout = channels[wiring["readout"][q[0]][0]]
        qubits[q].feedback = channels[wiring["feedback"][q[0]][0]]
    for q in ["B4", "B5"]:  # Qubits with LO around 7.5e9
        qubits[q].readout = channels[wiring["readout"][q[0]][1]]
        qubits[q].feedback = channels[wiring["feedback"][q[0]][1]]
